...
class TencentMultipartUploadManager {
  ...
  Future<bool> uploadFile(
    ...
    log('[TencentMultipartUploadManager] 开始流水线式上传 $totalChunks 个分块，并发数: $concurrency');
    ...
    await for (final chunk in reader.chunkStream(file)) {
      ...
      while (chunkBuffer.length >= concurrency || (nextIndex >= totalChunks - 1 && chunkBuffer.isNotEmpty)) {
        ...
        if (failedChunks.isNotEmpty) {
          logError('[TencentMultipartUploadManager] 有 ${failedChunks.length} 个分块重试后仍然失败');
          await abort();
          return false;
        }
  ...
  Future<bool> _uploadPartWithRetry(FileChunk chunk) async {
    ...
    while (retryCount < maxRetryCount) {
      ...
      if (retryCount < maxRetryCount) {
        log('[TencentMultipartUploadManager] 分块 ${chunk.partNumber} 上传失败，$retryCount/$maxRetryCount 次重试');
        // 等待后重试
        await Future.delayed(Duration(milliseconds: retryDelayMs));
      }
    }

    logError('[TencentMultipartUploadManager] 分块 ${chunk.partNumber} 重试 $maxRetryCount 次后仍然失败');
    return false;
  }
