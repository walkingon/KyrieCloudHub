...
设置界面增加设置下载目录功能，默认目录为 “设备的下载目录/KyrieCloudHubDownload/”，当用户修改下载目录后，下载目录变更为“用户选择的目录/KyrieCloudHubDownload/”。这里设置的下载目录为文件下载根目录，后续用户下载文件无需选择目录。

下载文件的保存路径为：KyrieCloudHubDownload/平台名/存储桶名/文件对象键 。例如：

- KyrieCloudHubDownload/Tencent/bucket-01/photo01.png
- KyrieCloudHubDownload/Ali/bucket-foo/images/bar.jpg

注意：当用户更改下载目录时，应用要检查用户选择的目录是否有写入权限，若无则给出提示并且设置无效。

### 存储桶对象界面修改

#### 下载操作修改

当用户选择对象进行“下载”或“批量下载”时，不再让用户选择保存路径，检查前面“设置界面修改”所述的路径中是否已经存在该文件，判断是否是相同文件： 1.同名 2.文件md5（尚未下载的云端文件能否获得其md5？）

- 若该云端文件尚未下载则进行下载
- 若该云端文件已下载，则给出提示并不再下载

#### 对象列表显示修改

对于已经下载过的相同文件，存储桶对象列表界面对于该文件对象的显示要有界面标识，表明该文件在本地已存在