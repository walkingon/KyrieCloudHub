...
2. ✅ 传输队列管理
   ...
   **已实现文件清单：**
   - `lib/services/transfer_queue_service.dart` - 传输队列服务（任务管理、并发控制、状态追踪）
   - `lib/screens/transfer_queue_screen.dart` - 传输队列UI界面（实时进度、清空功能）
   - `lib/main.dart` - 注册 TransferQueueService 到 Provider
...
3. ✅ 批量操作支持
   ...
   **已实现文件清单：**
   - `lib/screens/bucket_objects_screen.dart` - 文件列表界面（多选模式、批量操作）
...
**已实现文件清单：**
- `lib/services/api/ali_yun_oss_api.dart` - 阿里云OSS API实现类（签名、存储桶列表、对象操作、分块上传、批量删除）
...
2. 交互优化
   ...
   - 实现新建文件夹功能
      - 存储桶内加号（+）按钮原本是上传功能
      - 点击该加号按钮后做功能分裂，让用户选择是上传文件还是新建文件夹
      - 若用户选择新建文件夹，弹出对话框让用户输入文件夹名称，确定后完成在当前路径新建文件夹
   - 实现路径导航优化
   - 实现加载状态提示
   - 实现错误提示优化
...
**已实现文件清单：**
- [lib/screens/bucket_objects_screen.dart](lib/screens/bucket_objects_screen.dart) - 文件列表视图（支持列表/网格切换、文件类型图标差异化显示）
- [lib/screens/main_screen.dart](lib/screens/main_screen.dart) - 主界面（支持列表/网格切换存储桶显示）
