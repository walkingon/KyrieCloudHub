...
│   │   ├── multipart_download/  # 分块下载模块
│   │   │   └── multipart_download_manager.dart
│   │   └── webdav/              # WebDAV服务模块
│   │       ├── webdav_service.dart       # WebDAV服务管理器
│   │       └── webdav_server.dart        # WebDAV协议处理器和服务器
│   ├── widgets/                 # 公共组件
│   └── utils/                   # 工具类
│       ├── logger.dart
...
| **存储桶管理** | 查询、浏览存储桶 |
| **对象存储** | 文件上传(支持分片)、下载(支持分片)、删除 |
| **传输队列** | 任务队列管理，进度追踪 |
| **WebDAV服务** | 为存储桶提供WebDAV协议访问支持 |
| **本地存储** | 凭证持久化存储 |
